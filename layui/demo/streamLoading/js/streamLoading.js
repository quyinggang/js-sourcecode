 ...
 */
 ...
 (function(window, undefined) {
     ...
     utils.createLoadingContainer = function(streamLoading) {
         ...
         let container = utils.getElement('getElementsByClassName', 'loading'), node = null;
     ...
     utils.setFigure = function(options) {
         let FIGURE = 'figure', IMG = 'img', tagName = 'getElementsByTagName',
     ...
     SLoading.fn.start = function(id) {
         let streamLoading = utils.getElement('getElementById', id),
             settings = this.options, column = settings.column, initCount = column * 2,
             length = settings.images.length, end = length, clientHeight = streamLoading.clientHeight, 
             record = [0], timer = null;
         if (settings.images) {
             // 初始化时加载initCount条数据，
        ...
        //  处理自动滚动
         ...
         streamLoading.addEventListener('scroll', function() {
             ...
             end = figuresLength + column * 2 < length ? figuresLength + column * 2 : length;
