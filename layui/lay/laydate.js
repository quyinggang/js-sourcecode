...
layui.define(function(exports){
  ...
  Dates.run = function(options){
    ...
    // 如果是textarea或者input元素的话，就是value属性，否则就是innerHTML属性
    as.elemv = /textarea|input/.test(elem.tagName.toLocaleLowerCase()) ? 'value' : 'innerHTML';
    // 如果options中配置了init，就使用options.init的值，否则使用内置默认参数
    // elem中不包含value和innerHTML字符串，就返回当前日期
  ...
  Dates.orien = function(obj, pos){
    // getBoundingClientRect会返回对象，该对象中包含top、left等距离
    var tops, rect = Dates.elem.getBoundingClientRect();
    // 如果当前元素left小于(当前页面-250)数值
  ...
  Dates.view = function(elem, options){
    ...
    // 设置Dates函数的属性elem(绑定元素)、options(配置参数)、mm(日期范围)、mins(开始日期年月日等)、maxs(结束日期年月日等)
    Dates.elem = elem;
