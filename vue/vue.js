...
function parseHTML (html, options) {
  ...
  while (html) {
    ...
    if (!lastTag || !isPlainTextElement(lastTag)) {
      ...
      if (textEnd === 0) {
        ...
        if (startTagMatch) {
          // match的结果{ tagName, attrs, start, end }，开始处理
  ...
  function parseStartTag () {
    ...
    if (start) {
      ...
      var end, attr;
      // startTagClose：/^\s*(\/?)>/ 开始标签的结束处
      // attribute: /^\s*([^\s"'<>\/=]+)(?:\s*(=)\s*(?:"([^"]*)"+|'([^']*)'+|([^\s"'=<>`]+)))?/
      // 判断开始标签是否存在属性，循环处理到开始标签的结束处
      while (!(end = html.match(startTagClose)) && (attr = html.match(attribute))) {
        advance(attr[0].length);
        match.attrs.push(attr);
      }
      if (end) {
        // 判断是否是自闭合标签，即/>，如果存在，即unarySlash = '/'
  ...
  function handleStartTag (match) {
    ...
    // expectHTML默认是true
    if (expectHTML) {
      // 判断上一个处理的标签是否是p标签并且当前标签是非短语标签，即处理<p><div></div></p>这种不合法嵌套
    ...
    if (options.start) {
      // 调用传入parseHTML的options参数的start方法
...
function parse (
  ...
  parseHTML(template, {
    ...
    start: function start (tag, attrs, unary) {
      ...
      // apply pre-transforms 处理当前标签是input情况
      ...
      // v-pre存在时跳过这个元素和它子元素的编译过程，会不处理指令、属性等
      if (inVPre) {
        processRawAttrs(element);
      } else if (!element.processed) {
        // 处理v-for指令存在的情况
        processFor(element);
        // 处理v-if v-else-if v-else指令存在的情况
        processIf(element);
        // 处理v-once指令存在的情况
        processOnce(element);
        // 处理标签其他属性和指令
